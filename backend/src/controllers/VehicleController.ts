...
export const addVehicle = async (req: AuthRequest, res: Response) => {
    try {
        ...
        // Validare (OOP Exception - Nota 8)
        if (vin.length !== 17) throw new ValidationException("VIN invalid (trebuie 17 caractere)");

        const newVehicle = await VehicleModel.create(req.user.id, modelId, vin, plate, year);
