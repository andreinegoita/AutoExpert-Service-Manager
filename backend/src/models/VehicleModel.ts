...
export class VehicleModel {
  static async getAllByUser(userId: number) {
    const query = `
        SELECT v.id, v.plate_number, v.vin, v.manufacture_year, v.image_url, m.model_name, b.name as brand
        FROM vehicles v
  ...
  static async update(id: number, userId: number, data: any) {
    ...
    if (data.manufacture_year) {
      fields.push(`manufacture_year = $${idx++}`);
      values.push(data.manufacture_year);
    }
    if (data.image_url) {
